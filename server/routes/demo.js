...
// ── POST /api/demo/runs/:runId/email-tender-draft ─────────────────────────
// Markdown draft pack → same PDF shell as /api/export/pdf, then email PDF.
router.post('/runs/:runId/email-tender-draft', async (req, res) => {
  const { to, markdown, title, filename } = req.body ?? {};
  if (!to?.trim()) return res.status(400).json({ error: 'Recipient email is required.' });
  ...
  try {
    const packTitle = title?.trim() || 'Tender response draft pack';
    ...
    const { send: sendEmail } = require('../services/EmailService');
    const attachName = filename?.trim() || 'tender-response-draft-pack.pdf';
    await sendEmail({
      to:      to.trim(),
      subject: packTitle,
      html:    '<p>Please find the tender response draft pack (PDF) attached.</p><p>Generated by Curam Engineering AI Tender Response Generator (demo export).</p>',
      text:    'Tender response draft pack attached.\n\nGenerated by Curam Engineering AI Tender Response Generator (demo export).',
      attachments: [{ filename: attachName, content: pdfBuffer, contentType: 'application/pdf' }],
    });

    res.json({ ok: true });
  } catch (err) {
    console.error('[demo/email-tender-draft]', err.message);
    res.status(500).json({ error: `Failed to send draft pack: ${err.message}` });
  }
});
